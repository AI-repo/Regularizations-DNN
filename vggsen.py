...
#        with tf.name_scope("senterm"):
        self.sensitivityterm = tf.clip_by_value(self.loss, 0, 1) /(tf.clip_by_value(tf.abs(self.sen_coefficient),1,400) * tf.reduce_mean(tf.abs(self.fdot_prev))+ 1)
#            tf.summary.scalar("senterm", self.sensitivityterm)

        
#        with tf.name_scope("loss"):
        self.loss = self.loss + (self.flag1*tf.clip_by_value(self.sensitivityterm,0,5))
...
#        optimizer = tf.train.MomentumOptimizer(learning_rate = 0.001, momentum = 0.9).minimize(self.loss)
        ...
        tvars_opt1 = [var for var in tvars if 'sen_coe'  not in var.name]
        ...
        tvars_opt2 = [var for var in tvars if 'sen_coe'  in var.name]
            ...
            for epoch in range(start, epochs):
                ...
                for start, end in zip(range(0, len(input_train), batch_size), range(batch_size, len(input_train), batch_size)): 
                    ...
                    feed_dic1 = {self.images : read_images(input_train[start:end], img_size, batch_size),
                                ...
                                self.fdot_prev: grad_fdot, self.flag1:flagg1}   
                    
#                    _, batch_loss = sess.run([optimizer, loss], feed_dict = feed_dic ) 
                    _ , _, batch_loss, batch_accur, fdot_current, senterm = sess.run([train_step1 ,train_step2, self.loss , self.accuracy,
                                                                                    self.fdot_cur , self.sensitivityterm ],
                                                                                    feed_dict=feed_dic1)
                    if senterm<0.0005:
                        flagg1=[0]
                    ...
                    if batches_train%10 == 0:
                        print("ep %d,    batch:%d|%d  ,  train_loss : %f,   train_accuracy : %f ,senterm : %f " 
                      %(epoch+1, batches_train, total_batch_train, batch_loss, batch_accur,senterm))     
                ...
                feed_dic2 = {self.images : read_images(input_train[start:end], img_size, batch_size),
                            self.targets : np.reshape(targets_train[start:end], [batch_size, 1]),
                            self.fdot_prev: fdot_current, self.flag1:flagg1 }
...
#                        writer_train.flush()
                ...
                print(" senterm : %f ,      sencoef:%f  " %(senterm, senncoef))
