...
data_path = '/home/amin/Datasets/CRC/train'
...
def get_seg_images_path1(seg_path, Unicodes):
    count = 1 

    images_path = np.array([os.path.join(seg_path , x) for x in sorted(os.listdir(seg_path), key=natural_key)])
    
    for path in images_path:
        path_Unicode = os.path.split(path)[-1].split('.')[0].split('_')[1]
        
        try:
            target_index = np.reshape(Unicodes.index(path_Unicode),[-1])
            
        except:
            continue
        if target_index:
            path = np.reshape(path, [-1])    
            if count == 1 :
                images_address = path
                targets=target_index
            else :
                images_address = np.concatenate((images_address, path), 0)
                targets =np.concatenate((targets, target_index),0)
        count +=1
...
#        train_step1 = optimizer1.apply_gradients(grad_weight1)
        ...
        with tf.Session() as sess:
            writer_val = tf.summary.FileWriter('./logs/plot_val', sess.graph)
    ...
    #                    _, batch_loss = sess.run([optimizer, loss], feed_dict = feed_dic ) 
                    ...
                    if fold == n_splits:
                        summary1 = sess.run(merged, feed_dict = feed_dic)
                    ...
                    for start, end in zip(range(0, len(input_valid), batch_size), range(batch_size, len(input_valid), batch_size)):                
                        ...
                        for i in range(len(target_valid_b)):  
                            ...
                            if (np.argmax(np.array(pred_ans[i,]),0) == int(target_valid_b[i])):    
                                valid_accuracy += 1
                        valid_accuracy /= float(len(target_valid_b))
                        total_valid_acc += valid_accuracy
                        batches_valid +=1
                        print(" batches :%d|%d   , valid_accuracy : %f " %( batches_valid,total_batch_valid, valid_accuracy))  
                    
                    Avg_valid_acc = total_valid_acc/np.round(len(input_valid)/batch_size)
                    if fold == n_splits:
                        summary2 = sess.run(merged, feed_dict = feed_dic_valid)
                        writer_val.add_summary(summary2,epoch)
                        writer_val.flush()
#                    print("                                      valid_accuracy: %f"  %( Avg_valid_acc))  
...
#   Unicodes = [ os.path.split(folder)[-1].split('_')[-1] for folder in folders_path]
   ...
   seg_path = '/home/amin/Datasets/CRC/test'
#   seg_subpath = [os.path.join(seg_path, x) for x in sorted(os.listdir(seg_path), key=natural_key)]
   images_addresses, targets = get_seg_images_path1(seg_path, Unicodes)
...
#                                         4164,308,912,1208,1421,1556,1631,1808,2010,2068,2286]),[-1])
   ...
   #n_classes = 4219
   #n_classes = max(targets)+1 # here since the targets variable represents the target label of test samples we can not use it
   n_classes = len(folders_path)
...
#   targets = np.zeros(len(images_addresses)) 
   ...
   with tf.Session(config=tf.ConfigProto(log_device_placement=True)) as sess:
       ...
       for i in range(num_model):
           add = last_tunedweights_path + str(i) +'/model-1'
           ...
           for i in range(len(images_addresses)):
               ...
               pred_ans = sess.run(vgg.prob , feed_dict= {vgg.images : test_image, vgg.targets : test_targetvalue})
               maxi = pred_ans.argmax(); maxv = pred_ans[0,maxi]
               tensor = [[maxi,maxv]]
               pred_list.append(tensor)
        
       pred_out = np.array(pred_list)
       
       n_images = len(images_addresses)
       pred_out=np.reshape(pred_out,[(n_images*num_model), 2])
       s1 = pred_out[:n_images, :]; s2 = pred_out[n_images:(2*n_images), :]; s3 = pred_out[(2*n_images):(3*n_images), :]
       s4 = pred_out[(3*n_images):(4*n_images), :]; 
       s5 = pred_out[(4*n_images):(5*n_images), :]
       s6 = pred_out[(5*n_images):, :]
       s12 = np.dstack((s1,s2)); 
       s34 = np.dstack((s3,s4));
       s56 = np.dstack((s5,s6))
       s1234 = np.dstack((s12,s34)); 
       s12345 = np.dstack((s1234,s56))

       

#       pred_out_sum = pred_out.sum(axis = 0)
#       pred_output= pred_out/pred_out_sum[np.newaxis, :]
#       ensemble = np.mean(pred_out, axis=1)
       add =[]; targ=[]; predic=[]
       for j in range(len(images_addresses)):
           test_targetvalue = np.reshape(targets[j],[1,1])
           ens_out = s12345[j,:,:]
           pred= np.argmax(ens_out[1,:])
           output = int(ens_out[0,pred])
           
           if (int(output) == int(test_targetvalue)):    
                   accuracy += 1
                   print("accuracy:%d|%d" % (accuracy, len(images_addresses)))
           add.append(images_addresses[j]); targ.append(test_targetvalue); predic.append(output)    
       add=np.squeeze(np.array(add)); targ =np.squeeze(np.array(targ)); predic= np.squeeze(np.array(predic))
       s12 = np.dstack((add,targ)); 
       s34 = np.squeeze(np.dstack((s12,predic)));
            
           
#               myfile = open('/home/amin/models/output.txt', 'w')
#               with myfile:
#                   writer = csv.writer(myfile)
#                   writer.writerows(row)
           
       accu = accuracy / float(len(images_addresses))
       print("total accuracy:%f" %accu)   
#       
...
#vgg = vgg16(img_size, n_classes, batch_size, pretrained_weights)                      
#Ave_train_acc =vgg.train()

# Running test process

last_tunedweights_path = '/home/amin/models/tuned/'
pred_out= test(last_tunedweights_path)
