...
data_path = '/home/amin/Datasets/CRC/aug'
...
class vgg16(object):
    def __init__(self, img_size, n_classes, batch_size, pretrained_weights):
        ...
        self.sen_coe_initialvalue = 22
        self.rob_coe_initialvalue = 15
        self.lamda = 0.7
...
#        with tf.name_scope("senterm"):
        self.sensitivityterm = tf.clip_by_value(self.loss, 0, 1) /(tf.clip_by_value(tf.abs(self.sen_coefficient),1,500) * tf.reduce_mean(tf.abs(self.fdot_prev))+ 1)
#            tf.summary.scalar("senterm", self.sensitivityterm)
            
#        with tf.name_scope("robterm"):
        self.robustnessterm =  tf.clip_by_value(tf.abs(self.rob_coefficient),1,300) * tf.reduce_mean(tf.abs(self.fdot_prev))
#            tf.summary.scalar("robterm", self.robustnessterm)
        
#        with tf.name_scope("loss"):
        self.loss = (self.lamda*self.loss) + (1-self.lamda)*((self.flag1*tf.clip_by_value(self.robustnessterm,0,5)) 
                                                            + (self.flag2*tf.clip_by_value(self.sensitivityterm, 0,5)))
...
#        for var in tvars_opt2:
#            tf.assign(var,tf.clip_by_value(var,1, 300))
        
#        capped_tvars_opt2 = [(tf.clip_by_value(coe, 1,300)) for coe in tvars_opt2]
            ...
            for epoch in range(start, epochs):
                ...
                for start, end in zip(range(0, len(input_train), batch_size), range(batch_size, len(input_train), batch_size)):  
                    ...
                    feed_dic1 = {self.images : read_images(input_train[start:end], img_size, batch_size),
                                ...
                                self.fdot_prev: grad_fdot, self.flag1:flagg1, self.flag2: flagg2}   
                    
#                    _, batch_loss = sess.run([optimizer, loss], feed_dict = feed_dic ) 
                    _ , _, batch_loss, batch_accur, fdot_current, senterm, robterm = sess.run([train_step1 ,train_step2, self.loss , 
                                                                                               self.accuracy,self.fdot_cur , 
                                                                                               self.sensitivityterm , 
                                                                                               self.robustnessterm], feed_dict=feed_dic1)
                    if robterm<0.0005:
                        flagg1=[0]
                    if senterm<0.0005:
                        flagg2=[0]
                ...
                feed_dic2 = {self.images : read_images(input_train[start:end], img_size, batch_size),
                            self.targets : np.reshape(targets_train[start:end], [batch_size, 1]),
                            self.fdot_prev: fdot_current , self.flag1:flagg1, self.flag2: flagg2}
                
                weights_biases, regularizers, activations, grad_activ_value , lastbatch_acc = sess.run([tvars_opt1, tvars_opt2, self.activs,
                ...
                seg_path = '/home/amin/Datasets/seg_images'
            ...
            return  activations, grad_activ_value, grad_fdot,regularizers,weights_biases, Test_acc_list
#    
...
def test(last_tunedweights_path):
   ...
   seg_subpath = [os.path.join(seg_path, x) for x in sorted(os.listdir(seg_path), key=natural_key)]
   images_addresses, test_targets = get_seg_images_path(seg_subpath, Unicodes)
...
#                                         4164,308,912,1208,1421,1556,1631,1808,2010,2068,2286]),[-1])
   ...
   with tf.Session() as sess:
       ...
       for i in range(len(images_addresses)):
           ...
           test_targetvalue = np.reshape(test_targets[i],[1,1])
...
activations, grad_activ_value, grad_fdot ,regularizers,weights_biases,Test_acc_list=vgg.train()
